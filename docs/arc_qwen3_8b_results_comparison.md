...
本文档汇总当前 `result` 目录所有子文件夹中的 `summary.json`。共收录 **142** 组结果；所有 split 均为 `test`。
...
样本数：1172；汇总组数：5；重复次数：1；seeds：42。

| Method | Align method | Topology | Timing (total, s) ↓ | Accuracy ↑ | Text output tokens ↓ |
|---|---|---|---:|---:|---:|
| Baseline | identical | hierarchical | 4903.5047 | 90.7850% (0.907850) | 988,240 |
| Baseline | identical | sequential | 4960.4129 | 91.0410% (0.910410) | 976,465 |
| Latent MAS | linear | sequential | **4587.0494** | 87.7133% (0.877133) | **513,363** |
| Text MAS | identical | hierarchical | 20355.8815 | 93.1741% (0.931741) | 3,244,436 |
| Text MAS | identical | sequential | 13032.6507 | **93.6860% (0.936860)** | 2,321,736 |
...
样本数：164；汇总组数：13；重复次数：4；seeds：42, 43, 44, 45。
...
| Latent MAS | soft | sequential | 18429.5337 | 82.6219% (0.826219) | 2,469,034.75 |
...
样本数：378；汇总组数：13；重复次数：4；seeds：42, 43, 44, 45。
...
| Latent MAS | kernel | hierarchical | 7599.0738 | 72.7514% (0.727514) | 567,826 |
| Latent MAS | kernel | sequential | 6781.2262 | 75.2645% (0.752645) | 552,575 |
| Latent MAS | kernel_early_stopping | hierarchical | 7640.8815 | 72.1561% (0.721561) | 551,998 |
| Latent MAS | kernel_early_stopping | sequential | 7401.6131 | 75.1984% (0.751984) | 577,527.75 |
...
| Latent MAS | soft | hierarchical | 34864.3366 | 73.2143% (0.732143) | 4,826,442.50 |
| Latent MAS | soft | sequential | 38387.1590 | 73.1482% (0.731482) | 5,438,771.50 |
...
| Latent MAS | identical | hierarchical | 8803.3404 | 76.9180% (0.769180) | **457,003.25** |
| Latent MAS | identical | sequential | 8263.1234 | 78.1746% (0.781746) | 508,530.75 |
| Latent MAS | linear | hierarchical | 12000.8109 | 8.2011% (0.082011) | 1,297,529 |
| Latent MAS | linear | sequential | 9544.8428 | 52.9101% (0.529101) | 598,486 |
| Text MAS | identical | hierarchical | 38095.0525 | 80.4894% (0.804894) | 2,150,787.75 |
| Text MAS | identical | sequential | 21867.2596 | **81.8783% (0.818783)** | 1,284,408.25 |

## MedQA

### Qwen/Qwen3-8B

样本数：300；汇总组数：13；重复次数：1；seeds：42。
...
| Latent MAS | kernel | hierarchical | 4713.8600 | 73.3333% (0.733333) | 405,393 |
| Latent MAS | kernel | sequential | **3744.6138** | 73.6667% (0.736667) | **341,569** |
| Latent MAS | kernel_early_stopping | hierarchical | 4438.8190 | 72.6667% (0.726667) | 400,776 |
| Latent MAS | kernel_early_stopping | sequential | 4066.8033 | 72.0000% (0.720000) | 346,273 |
...
| Latent MAS | soft | sequential | 33334.8013 | 66.6667% (0.666667) | 4,791,457 |
...
样本数：300；汇总组数：7；重复次数：1；seeds：42。

| Method | Align method | Topology | Timing (total, s) ↓ | Accuracy ↑ | Text output tokens ↓ |
|---|---|---|---:|---:|---:|
| Baseline | identical | hierarchical | 5647.1969 | **81.6667% (0.816667)** | 448,903 |
| Baseline | identical | sequential | 5788.0867 | 80.3333% (0.803333) | 454,684 |
| Latent MAS | identical | hierarchical | **3635.8131** | 81.3333% (0.813333) | **260,894** |
| Latent MAS | identical | sequential | 5714.4184 | 81.0000% (0.810000) | 484,708 |
| Latent MAS | linear | hierarchical | 8335.6065 | 58.0000% (0.580000) | 455,017 |
...
| Method | Align method | Topology | Timing (total, s) ↓ | Accuracy ↑ | Text output tokens ↓ |
|---|---|---|---:|---:|---:|
| Latent MAS | kernel | hierarchical | 7752.0201 | 60.0000% (0.600000) | 297,770.50 |
| Latent MAS | kernel | sequential | **6767.4067** | **66.2500% (0.662500)** | **256,437.50** |
| Latent MAS | kernel_early_stopping | hierarchical | 7735.4584 | 65.8333% (0.658333) | 296,946 |
...
样本数：30；汇总组数：1；重复次数：8；seeds：42, 43, 44, 45, 46, 47, 48, 49。

| Method | Align method | Topology | Timing (total, s) ↓ | Accuracy ↑ | Text output tokens ↓ |
|---|---|---|---:|---:|---:|
| Latent MAS | kernel | sequential | **7668.5074** | **72.0833% (0.720833)** | **247,758.75** |
...
数据快照日期：2026-08-24。仅收录实际存在且可解析的 `summary.json`。
