...
- 当前状态：本地 cross-vocab STT 配置/schema/cache/runtime、四个 task、6/12/3/1 matrix、Slurm 链与 cache-only 统计报告均已实现；干净 `origin/main` 基线上 `analysis/tests` 34 项全部通过。两个 runtime-v2 artifacts 均已在锁定 revision 的真实 tokenizer 上通过 strict gate，正反向 source support 都完整。
- 当前计划：推送 runtime-normalization 工具与配置更新；通过 `scp` 将两个 Git-ignored 大 artifact 放到 Guqq，在远端 pull 成功后通过 Slurm 依次运行双向 smoke、三数据集 integration smoke、12 个正式单元和最终报告。
