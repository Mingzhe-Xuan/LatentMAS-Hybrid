...
if [[ -z "${PBS_ARRAY_INDEX:-}" ]]; then
    ...
    echo "Submitted ${JOB_ID}: 84 slow configs in ${ARRAY_JOB_COUNT} GPU jobs, ${TASKS_PER_GPU} parallel configs per GPU, maximum 3 concurrent GPU jobs, force_all=${FORCE_ALL}."
    exit 0
fi
