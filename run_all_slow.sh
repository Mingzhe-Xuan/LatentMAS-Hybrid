...
PROGRESS_FILE="${PROGRESS_FILE:-${SUBMIT_DIR}/state_slow.txt}"
...
if [[ -z "${PBS_ARRAY_INDEX:-}" ]]; then
    ...
    ARRAY_JOB_COUNT=$(((84 + TASKS_PER_GPU - 1) / TASKS_PER_GPU))
    VARIABLES="SLOW_ONLY=true,FORCE_ALL=${FORCE_ALL},PROGRESS_FILE=${PROGRESS_FILE},TASKS_PER_GPU=${TASKS_PER_GPU},MAX_SAMPLES=${MAX_SAMPLES},KERNEL_FEATURES=${KERNEL_FEATURES},KERNEL_TEMPERATURE=${KERNEL_TEMPERATURE},KERNEL_CHUNK_SIZE=${KERNEL_CHUNK_SIZE},ALIGN_RIDGE=${ALIGN_RIDGE},SOFT_TEMPERATURE=${SOFT_TEMPERATURE},SOFT_CHUNK_SIZE=${SOFT_CHUNK_SIZE},EARLY_STOPPING_LENGTH_THRESHOLD=${EARLY_STOPPING_LENGTH_THRESHOLD},EARLY_STOPPING_ENTROPY_THRESHOLD=${EARLY_STOPPING_ENTROPY_THRESHOLD}"
    JOB_ID="$(cd "${SCRIPT_DIR}" && qsub -J "1-${ARRAY_JOB_COUNT}%3" -v "${VARIABLES}" "${BASH_SOURCE[0]}")"
    echo "Submitted ${JOB_ID}: 84 slow configs in ${ARRAY_JOB_COUNT} GPU jobs, one config per GPU, maximum 3 concurrent GPU jobs, force_all=${FORCE_ALL}."
    exit 0
fi

export SLOW_ONLY=true FORCE_ALL PROGRESS_FILE TASKS_PER_GPU MAX_SAMPLES KERNEL_FEATURES
