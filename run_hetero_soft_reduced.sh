...
# Cross-model exact-Soft latent-alignment jobs for three selected datasets.
...
PROGRESS_FILE="${PROGRESS_FILE:-${SUBMIT_DIR}/state_hetero_soft_reduced.txt}"
...
for arg in "$@"; do
    case "${arg}" in
        ...
        *)
            ...
            echo "Usage: bash run_hetero_soft_reduced.sh [--force_all]" >&2
            exit 2
            ;;
    esac
done

# Reduced benchmark subset requested for the Soft rerun.
DATASETS=(aime2024 humanevalplus medqa)
